...
Entity_MarkerType::Entity_MarkerType(int x, int y, int w, int h)
{
    this->type = "MarkerType"; //we need the type because it counteracts the ambiguity of polymorphism
    this->x = x;
    this->y = y;
    this->w = w;
    this->h = h;
...
bool Entity_MarkerType::serialize(fstream& file)
{
...
bool Entity_MarkerType::unserialize(ifstream& file)
{
    if(!file.is_open()) return false;
    int entity_version;
