...
Entity_ButtonType_Black::Entity_ButtonType_Black(int x, int y, int w, int h)
{
    this->type = "ButtonType_Black"; //we need the type because it counteracts the ambiguity of polymorphism
    this->x = x;
    this->y = y;
    this->w = w;
    this->h = h;
...
bool Entity_ButtonType_Black::serialize(fstream& file)
{
    ...
    int Entity_ButtonType_Black_version = 1;
    file.write((char*) &Entity_ButtonType_Black_version, sizeof(Entity_ButtonType_Black_version));
...
bool Entity_ButtonType_Black::unserialize(ifstream& file)
{
    if(!file.is_open()) return false;
    int Entity_ButtonType_Black_version;
    file.read((char*) &Entity_ButtonType_Black_version, sizeof(int));
    if(Entity_ButtonType_Black_version==1){
