...
class Texture{
    public:
    bool load(std::string filename, bool wrap = false);
...
Texture LoadTexture(std::string filename, bool wrap = false); //A non-class function for creating a Texture object and loading it at the same time;
