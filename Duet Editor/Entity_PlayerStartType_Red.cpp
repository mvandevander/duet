...
Entity_PlayerStartType_Red::Entity_PlayerStartType_Red(int x, int y, int w, int h)
{
    this->type = "PlayerStartType_Red"; //we need the type because it counteracts the ambiguity of polymorphism
    this->x = x;
    this->y = y;
    this->w = w;
    this->h = h;
...
bool Entity_PlayerStartType_Red::serialize(fstream& file)
{
...
bool Entity_PlayerStartType_Red::unserialize(ifstream& file)
{
    if(!file.is_open()) return false;
    int entity_version;
