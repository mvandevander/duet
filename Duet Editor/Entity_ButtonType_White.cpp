...
Entity_ButtonType_White::Entity_ButtonType_White(int x, int y, int w, int h)
{
    this->type = "ButtonType_White"; //we need the type because it counteracts the ambiguity of polymorphism
    this->x = x;
    this->y = y;
    this->w = w;
    this->h = h;
...
bool Entity_ButtonType_White::serialize(fstream& file)
{
    ...
    int Entity_ButtonType_White_version = 1;
    file.write((char*) &Entity_ButtonType_White_version, sizeof(Entity_ButtonType_White_version));
...
bool Entity_ButtonType_White::unserialize(ifstream& file)
{
    if(!file.is_open()) return false;
    int Entity_ButtonType_White_version;
    file.read((char*) &Entity_ButtonType_White_version, sizeof(int));
    if(Entity_ButtonType_White_version==1){
