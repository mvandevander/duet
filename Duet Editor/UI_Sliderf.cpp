...
UI_Sliderf::UI_Sliderf(string label, struct Font* font, int x, int y, int w, int h, float *var_to_manipulate, float low, float high)
{
    this->x = x;
    this->y = y;
    this->w = w;
...
void UI_Sliderf::update(SDL_Event event)
{
    float total_range = high-low;
    current_percent = (*var_to_manipulate - low)/total_range;
    ...
    if(slider_is_grabbed){
        ...
        float final_val = (percent*total_range)+low;
