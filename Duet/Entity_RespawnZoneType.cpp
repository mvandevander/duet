...
Entity_RespawnZoneType::Entity_RespawnZoneType(int x, int y, int w, int h)
{
    this->type = "RespawnZoneType"; //we need the type because it counteracts the ambiguity of polymorphism
    this->x = x;
    this->y = y;
    this->w = w;
    this->h = h;
    ...
    this->index = 0;
    red_player_activated = false;
    green_player_activated = false;
}
...
bool Entity_RespawnZoneType::serialize(fstream& file)
{
...
bool Entity_RespawnZoneType::unserialize(ifstream& file)
{
    if(!file.is_open()) return false;
    int entity_version;
