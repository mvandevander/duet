...
Entity_DeathLaserType::Entity_DeathLaserType(int x, int y, int w, int h)
{
    this->type = "DeathLaserType"; //we need the type because it counteracts the ambiguity of polymorphism
    this->x = x;
    this->y = y;
    this->w = w;
    this->h = h;
    ...
    this->firing_delay_time = 0;
    this->laser_lifetime = 0;
    this->laser_beam_length = 0;
}
...
bool Entity_DeathLaserType::unserialize(ifstream& file)
{
    if(!file.is_open()) return false;
    int Entity_DeathLaserType_version;
    file.read((char*) &Entity_DeathLaserType_version, sizeof(int));
    if(Entity_DeathLaserType_version==1){
