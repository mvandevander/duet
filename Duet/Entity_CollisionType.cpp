...
Entity_CollisionType::Entity_CollisionType(int x, int y, int w, int h)
{
    this->type = "CollisionType"; //we need the type because it counteracts the ambiguity of polymorphism
    this->x = x;
    this->y = y;
    this->w = w;
    this->h = h;
...
bool Entity_CollisionType::serialize(fstream& file)
{
    ...
    int entity_collisiontype_version = 1;
    file.write((char*) &entity_collisiontype_version, sizeof(entity_collisiontype_version));
...
bool Entity_CollisionType::unserialize(ifstream& file)
{
    if(!file.is_open()) return false;
    int entity_collisiontype_version;
    file.read((char*) &entity_collisiontype_version, sizeof(int));
    if(entity_collisiontype_version==1){
