...
Entity_PlayerStartType_Green::Entity_PlayerStartType_Green(int x, int y, int w, int h)
{
    this->type = "PlayerStartType_Green"; //we need the type because it counteracts the ambiguity of polymorphism
    this->x = x;
    this->y = y;
    this->w = w;
    this->h = h;
...
bool Entity_PlayerStartType_Green::serialize(fstream& file)
{
...
bool Entity_PlayerStartType_Green::unserialize(ifstream& file)
{
    if(!file.is_open()) return false;
    int entity_version;
