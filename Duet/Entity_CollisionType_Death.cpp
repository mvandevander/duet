...
Entity_CollisionType_Death::Entity_CollisionType_Death(int x, int y, int w, int h)
{
    this->type = "CollisionType_Death"; //we need the type because it counteracts the ambiguity of polymorphism
    this->x = x;
    this->y = y;
    this->w = w;
    this->h = h;
...
bool Entity_CollisionType_Death::serialize(fstream& file)
{
    ...
    int Entity_CollisionType_Death_version = 1;
    file.write((char*) &Entity_CollisionType_Death_version, sizeof(Entity_CollisionType_Death_version));
...
bool Entity_CollisionType_Death::unserialize(ifstream& file)
{
    if(!file.is_open()) return false;
    int Entity_CollisionType_Death_version;
    file.read((char*) &Entity_CollisionType_Death_version, sizeof(int));
    if(Entity_CollisionType_Death_version==1){
