...
Entity_CollisionType_Green::Entity_CollisionType_Green(int x, int y, int w, int h)
{
    this->type = "CollisionType_Green"; //we need the type because it counteracts the ambiguity of polymorphism
    this->x = x;
    this->y = y;
    this->w = w;
    this->h = h;
...
bool Entity_CollisionType_Green::serialize(fstream& file)
{
    ...
    int Entity_CollisionType_Green_version = 1;
    file.write((char*) &Entity_CollisionType_Green_version, sizeof(Entity_CollisionType_Green_version));
...
bool Entity_CollisionType_Green::unserialize(ifstream& file)
{
    if(!file.is_open()) return false;
    int Entity_CollisionType_Green_version;
    file.read((char*) &Entity_CollisionType_Green_version, sizeof(int));
    if(Entity_CollisionType_Green_version==1){
